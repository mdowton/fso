...
if (!class_exists('ModuleContacts')) :
    class ModuleContacts extends MagenestZohoConnect {
        public function __construct(){
            ...
            global $wpdb;
            $query = "SELECT DISTINCT `wooc_action` FROM `{$wpdb->prefix}hn_zoho_crm_field_mapping` WHERE `zoho_module` = 'Contacts'";
            ...
            foreach ($hookActions as $hookAction){
                ...
                if($sync == 'queue') {
                    add_action($hookAction['wooc_action'], array($this, 'ContactsQueue'));
                } else {
                    add_action($hookAction['wooc_action'], array($this, 'ProcessContacts'));
                }
            }
         ...
         */
        public function ProcessContacts($id){
            $resultSend = $this->sendCurlRequest("insertRecords",$this->PrepareXml($id));
            // insert report
            if ($resultSend['zoho_id'] == false){
                // sync error
                $this->insert_report($id, 0, "Sync to Contacts", 0, "Sync error: " . $resultSend['zoho_mess']);
            } else {
                $this->insert_report($id, $resultSend['zoho_id'], "Sync to Contacts", 1, $resultSend['zoho_mess']);
            }
        }

        public function ContactsQueue($id){
            if ( $id instanceof WP_User ) {
                $this->insert_report($id, 0, "Account Sync to Contacts", 0, __("Not yet be synchronized"));
            } else {
                $this->insert_report($id, 0, "Order Sync to Contacts", 0, __("Not yet be synchronized"));
            }
        }
        ...
        public function PrepareXml($id){
            ...
            $xml .= $processOrder->Mapping($data , "Contacts");
            $xml.= '</row></Contacts>';
