...
if ( is_admin()){
	add_filter('topz-opts-rights', 'add_field_rights', 10, 2);
}

function topz_options(){
	global $topz_options;
	return $topz_options;
}

$add_query_vars = array();
function topz_query_vars( $qvars ){
...
if (!is_admin()){
	add_filter('query_vars', 'topz_query_vars');
	add_action('parse_request', 'topz_parse_request');
}
?>