...
class WooCommerce_Widget_Currency_Converter extends WP_Widget {
	...
	function __construct() {
		...
		$this->woo_widget_description = esc_html__( 'Allow users to choose a currency for prices to be displayed in.', 'topz' );
		$this->woo_widget_idbase = 'woocommerce_currency_converter';
		$this->woo_widget_name = esc_html__('WooCommerce Currency Converter', 'topz' );
			...
			<ul class="currency_w">
				...
				<?php					
					...
					if ( $currencies ) {
						...
						echo '</ul></li>';
						if ( $show_reset )
							echo '<li><a href="#" class="reset">' . esc_html__('Reset', 'topz') . '</a></li>';
	...
	function form( $instance ) {
		global $wpdb;
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title:', 'topz') ?></label>
		<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" value="<?php if (isset ( $instance['title'])) {echo esc_attr( $instance['title'] );} else {echo esc_html__('Currency converter', 'topz');} ?>" /></p>

		<p><label for="<?php echo esc_attr( $this->get_field_id('currency_codes') ); ?>"><?php esc_html_e('Currency codes:', 'topz') ?> <small>(<?php esc_html_e('1 per line', 'topz') ?>)</small></label>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id('show_reset') ); ?>"><?php esc_html_e('Show reset link:', 'topz') ?></label>
