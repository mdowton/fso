...
class MRP_Utils {
	 ...
	 * @param $post_id
	 */
	public static function cookie_validation_check( $post_id, $rating_form_id ) {
		return isset( $_COOKIE[MRP_Multi_Rating::POST_SAVE_RATING_COOKIE . '-' . $rating_form_id . '-' . $post_id] );
	}
	
	/**
	 * Check IP address has not saved a rating form with the post ID within specified hours
	 */
	public static function ip_address_validation_check( $ip_address, $post_id, $rating_form_id ) {
		
		global $wpdb;
		$general_settings = (array) get_option( MRP_Multi_Rating::GENERAL_SETTINGS );
				
		// check IP address has not submitted a rating for the post ID within a specified time limit
		...
		$date_compare = date( 'Y-m-d H:i:s', strtotime( current_time('mysql') ) - $time_limit );
		
		$query = 'SELECT * FROM ' . $wpdb->prefix . MRP_Multi_Rating::RATING_ITEM_ENTRY_TBL_NAME . ' WHERE ip_address = %s'
					. ' AND post_id = %d AND rating_form_id = %d AND entry_date >= %s';

		$rows = $wpdb->get_results( $wpdb->prepare( $query, $ip_address, $post_id, $rating_form_id, $date_compare ) );
	...
	public static function duplicate_rating_check( $post_id, $rating_form_id ) {
		...
		foreach ( $save_rating_restriction_types as $save_rating_restriction_type ) {
			...
			if ( ( $save_rating_restriction_type == 'ip_address' && MRP_Utils::ip_address_validation_check( $ip_address, $post_id, $rating_form_id ) == true )
					|| ( $save_rating_restriction_type == 'cookie' && MRP_Utils::cookie_validation_check( $post_id, $rating_form_id ) == true ) ) {
				return true;
			}
		}
