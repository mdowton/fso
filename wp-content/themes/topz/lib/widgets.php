 ...
 */
function topz_unregister_default_widgets() {
...
add_action('widgets_init', 'topz_unregister_default_widgets', 11);
 
/*
** Register Sidebar and Widgets
*/
function topz_widgets_init() {
	
	// Sidebars
	global $topz_widget_areas;
	$topz_widget_areas = topz_widget_setup_args();
	if ( count($topz_widget_areas) ){
		foreach( $topz_widget_areas as $sidebar ){
			$sidebar_params = apply_filters('topz_sidebar_params', $sidebar);
			register_sidebar($sidebar_params);
		}
	}

	// Widgets
	register_widget('Topz_Social_Widget');
	register_widget('Topz_Posts_Widget');
	register_widget('Topz_Vertical_Megamenu_Widget');
	register_widget('Topz_Top_Widget');
}
add_action('widgets_init', 'topz_widgets_init');
 ...
 * @since 2.8.0
*/
class Topz_Posts_Widget extends Topz_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'sw_posts', 'description' => esc_html__('SW Posts', 'topz'));
		parent::__construct('sw_posts', esc_html__('SW Posts', 'topz'), $widget_ops);
		$this->base = get_template_directory().'/lib';
	}
}

class Topz_Vertical_Megamenu_Widget extends Topz_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'vertical_megamenu', 'description' => esc_html__('SW Vertical Mega Menu Widget', 'topz'));
		parent::__construct('vertical_megamenu', esc_html__('SW Vertical Mega Menu', 'topz'), $widget_ops);
	}
}


class Topz_Top_Widget extends Topz_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'sw_top', 'description' => esc_html__('SW top header widget', 'topz'));
		parent::__construct('sw_top', esc_html__('SW Top Widget', 'topz'), $widget_ops);
	}
}

class Topz_Social_Widget extends WP_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'sw_social', 'description' => esc_html__('SW Social Networks', 'topz'));
		parent::__construct('sw_social', esc_html__('SW Social', 'topz'), $widget_ops);
	...
	function form($instance){ 
		...
		?>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title: ', 'topz'); ?>
...
<?php 
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo esc_html__( 'Social', 'topz' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'topz')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'topz')?> </label>
...
<?php
		...
		// topz fields for add new social network
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'topz' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'topz'); ?>
	</label> <input class="widefat" id="<?php echo esc_attr( $id_i_icon ); ?>"
		name="<?php echo esc_attr( $name_i_icon ); ?>" type="text" value=""
		placeholder="<?php esc_attr_e( 'Enter Font Awesome icon or Title', 'topz' ) ?>" /> <span><?php esc_html_e( 'If using as
		icon, please choose class name in Font Awesome. This is required.', 'topz' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'topz')?> </label>
...
class Topz_Widgets{
	...
	protected $fields = array(
			'topz_before_widget' => 'Before Widget',
			'topz_after_widget'  => 'After Widget',
			'topz_before_title'  => 'Before Title',
			'topz_after_title'   => 'After Title',
			'topz_display_conditions' => 'Display Conditions',
	...
	public function widget_display_callback( $instance, $widget, $args ){		
		...
		if ( topz_check_wdisplay($widget_display) == false ) return false;
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		$widget_advanced = topz_options()->getCpanelValue('widget-advanced');
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'topz' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'topz' ) ?>
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => esc_html__('All', 'topz'),
				'if_selected' => esc_html__('Only on selected', 'topz'),
				'if_no_selected' => esc_html__('Except selected', 'topz')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'topz' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => esc_html__('Show only for login users' , 'topz'),
				'logout' => esc_html__('Show only for logout users' , 'topz')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'topz' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => esc_html__('Blog posts', 'topz'),
					'archive' => esc_html__('Archive page', 'topz'),
					'date' => esc_html__('Archive Date', 'topz'),
					'category' => esc_html__('Category archive', 'topz'),
					'tag' => esc_html__('Tag archive', 'topz'),
					'author' => esc_html__('Author archive', 'topz'),
					'search' => esc_html__('Search page', 'topz'),
					'single' => esc_html__('Single post', 'topz'),
					'page' => esc_html__('Page template', 'topz'),
					'404' => esc_html__('Page 404', 'topz')
				);
			$html = '';
				...
				if(!isset($menu) || $menu != NULL || $menu != "")
					{
						...
						if (count($items) > 0) {
							$menu_checkbox = new Topz_Menu_Checkbox( $menu->slug);
	...
	protected function getWidgetStyles(){
		...
		if ( is_null($this->styles) ){
			$tmp = array();
			if ( $_core_styles = $this->scandir(TOPZ_DIR.'/widgets/_styles', 'php') )
	...
	protected function loadStyle( $style = '' ){
		...
		if ( !empty($style) ){
			...
			$_core_style = TOPZ_DIR.'/widgets/_styles/'.$style.'.php';
