...
if ( ! class_exists( 'ZohoQueue' ) ) :
    class ZohoQueue  {
        public function output(){
            echo '<h2>Queue table: Module Leads</h2>';
            $this->showModule('Leads');
            echo '<h2>Queue table: Module Accounts</h2>';
            $this->showModule('Accounts');
            echo '<h2>Queue table: Module Contacts</h2>';
            $this->showModule('Contacts');
            echo '<hr/>';
            echo '<h2>Queue table: Module Invoices</h2>';
            $this->showModule('Invoices');
            echo '<h2>Queue table: Module SalesOrders</h2>';
            $this->showModule('SalesOrders');
            echo '<h2>Queue table: Module PurchaseOrders</h2>';
            $this->showModule('PurchaseOrders');
            echo '<hr/>';
            echo '<h2>Queue table: Module Products</h2>';
            ...
            foreach ( $rows as $row ) {
                ...
                <tr>
                    ...
                    <td><form action="admin-post.php" method="post">
                            ...
                            <input type="submit" class="button" name="btnSubmit" value="Sync"/>
        ...
        public function get_collection($module) {
            ...
            $query = "SELECT * FROM {$mail_queue_table} WHERE `status` = '0' AND `mess` LIKE '%Not yet be synchronized%' AND `type` LIKE '%{$module}%' ORDER BY `id` DESC";
