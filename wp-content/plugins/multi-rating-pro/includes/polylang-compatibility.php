...
function mrp_polylang_query_where_post( $query_where, $params, $table_prefix = null ) {
	...
	if ( $all_language_translations ) { // all translated posts
		$translations = maybe_unserialize( $wpdb->get_var( $wpdb->prepare( 'SELECT description FROM ' . $wpdb->prefix . 'term_relationships rel, wp_term_taxonomy tax WHERE rel.object_id = %d AND tax.taxonomy = "post_translations" AND tax.term_taxonomy_id = rel.term_taxonomy_id', $post_id ) ) );
