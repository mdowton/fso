...
class MRP_Multi_Rating_API {
	...
	public static function get_rating_entry_result_list( $params = array() ) {
		...
		*/
		...
		$results = $wpdb->get_results( $wpdb->prepare( $missing_query, $query_args ) );
		...
		if ( count( $results ) > 0 ) {
			...
			$entry_values_query = 'SELECT riev.rating_item_entry_id AS rating_entry_id, riev.rating_item_id, riev.value, rie.post_id, '
					...
					. ' IN ( ' . implode( ', ', $temp_rating_entry_ids ) . ' ) ORDER BY riev.rating_item_entry_id';

			$rows = $wpdb->get_results( $entry_values_query );
	...
	public static function save_rating_entry( $rating_entry, $validate_fields = true ) {
		...
		$ip_address = isset( $rating_entry['ip_address'] ) ? $rating_entry['ip_address'] : null;
		...
		if ( $is_new ) {
			...
			if ( isset( $ip_address ) ) {
				$insert_data = array_merge( $insert_data, array( 'ip_address' => $ip_address ) );
				array_push( $insert_data_format, '%s' );
			}
