...
class Topz_Menu_Admin {
	...
	static function options_setup() {
		self::$custom_fields = array(
			'mega_active' => array(
					'type' => 'checkbox',
					'label' => esc_html__( 'Active mega menu', 'topz' ),
					'default' => 0					
			),
			'icon' => array(
					'type' => 'text',
					'label' => 'Icon',
					'description' => sprintf( __('Show an icon by <a href="%s">Font Awesome 4.6</a>','topz'), esc_url( 'http://fortawesome.github.com/Font-Awesome/' ) ),
					'default' => '',
					'preview' => true,
			),
			'span' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Dropdown sub size', 'topz' ),
					'default' => '',
					'options' => array(
						'' => 'default',
						'col-1' => '1',
			...
			'mega_full' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Active Full Mega Menu', 'topz' ),
					'default' => 0					
			),
			'dropdown_span' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Dropdown Size', 'topz' ),
					'default' => '',
					'options' => array(
						'column-1' => '1',
						'column-2' => '2',
			...
			'show_description_as_subtitle' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Show Description as Subtitle', 'topz' ),
					'default' => 0					
			),
			'hide_title' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Hide Title', 'topz' ),
					'description' =>  esc_html__( 'If icon or subtitle are using.', 'topz' ),
					'default' => 0
			),
			'disable_link' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Disable Link', 'topz' ),
					'description' =>  esc_html__( 'Remove Link', 'topz' ),
					'default' => 0
			),
			'advanced' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Advanced', 'topz' ),
					'default' => '',
					'options' => array(
							'no' =>  esc_html__( 'Default', 'topz' ),							
							'apc' =>  esc_html__( 'Append Advanced Content or Shortcode', 'topz' ),
							'apcs' =>  esc_html__( 'Append Advanced Content with Page Select', 'topz' )
					)
			),
			'advanced_content' => array(
					'type' => 'textarea',
					'label' =>  esc_html__( 'Advanced Content', 'topz' ),
					'default' => '',
					'depends' => 'advanced_eq_rbd'
			),
			'page_select' => array(
					'type' => 'page_select',
					'label' =>  esc_html__( 'Page Select', 'topz' ),
					'default' => 0,
					'description' =>  esc_html__( 'Select page to show on mega menu', 'topz' ),					
			),
			'imgupload'		   => array(
					'type'	=> 'upload',
					'label' =>  esc_html__( 'Menu Thumbnail', 'topz' ),
					'default' => ''
			),
		);
	}
}
add_action( 'admin_init', array('Topz_Menu_Admin','options_setup' ) );
if ( !class_exists('Topz_Menu') ):
...
class Topz_Menu {
	public function __construct(){
		...
		add_action( 'admin_enqueue_scripts', array( $this,'topz_menu_upload') );
	}
	function topz_menu_upload(){
		wp_enqueue_script(
			'topz-menu-upload-js',get_template_directory_uri().'/lib/admin/js/field_upload.js',
	...
	public function add_custom_nav_fields( $menu_item ){
		if ( count(Topz_Menu_Admin::$custom_fields) ){
			foreach (Topz_Menu_Admin::$custom_fields as $field => $field_opts){
	...
	public function update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ){
		if ( count(Topz_Menu_Admin::$custom_fields) ){
			foreach (Topz_Menu_Admin::$custom_fields as $field => $type){
	...
	public function edit_walker($walker, $menu_id){
		return 'Topz_Menu_Admin_Walker';
	}
}

endif;

if ( !class_exists('Topz_Menu_Admin_Walker') ):
 ...
 */
class Topz_Menu_Admin_Walker extends Walker_Nav_Menu  {
	/**
	...
	function start_el(&$output, $item, $depth = 0, $args = Array(), $current_object_id = 0) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'topz' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__( '%s (Pending)','topz' ), $item->title );
		}
				...
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'topz'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'topz'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item', 'topz'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php esc_html_e( 'Edit Menu Item', 'topz' ); ?></a>
					</span>
				</dt>
			</dl>
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Navigation Label','topz' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Title Attribute','topz' ); ?><br />
						<input type="text" id="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
					</label>
				</p>
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
						<input type="checkbox" id="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>" value="_topz" name="menu-item-target[<?php echo esc_attr( $item_id ); ?>]"<?php checked( $item->target, '_topz' ); ?> />
						<?php esc_html_e( 'Open link in a new window/tab','topz' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'CSS Classes (optional)','topz' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)','topz' ); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Description','topz' ); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr( $item_id ); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','topz'); ?></span>
					</label>
				</p>
				<div class="advance-menu">
					<a href="javascript:void(0);" class="menu-advance-href" data-id="menu-advance-<?php  echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Show/Hide Advance Menu Config', 'topz'); ?> </a>
								...
								switch( $field_opts['type'] ):
									...
									case 'select':
										...
										<?php if ( isset($field_opts['options']) && is_array($field_opts['options']) ): ?>
											...
											><?php echo ( sprintf( _n( '%s', '%s', $opt_label, 'topz' ), $opt_label ) ); ?></option>
									...
									case 'page_select':
									?><select  id="edit-menu-item-<?php echo esc_attr( $field ); ?>-<?php echo esc_attr( $item_id ); ?>" class="edit-menu-item-<?php echo esc_attr( $field ); ?>" name="menu-item-<?php echo esc_attr( $field ); ?>[<?php echo esc_attr( $item_id ); ?>]" >
										<option value="0" selected><?php esc_html_e( 'Select page', 'topz' ); ?></option>
										<?php $pages = get_pages(); ?>
									...
									?>			
											...
											<?php
												echo ( $item->$field != '') ? '<img class="topz-opts-screenshot" id="topz-opts-screenshot-'.esc_attr( $item_id ).'" src="'.esc_attr( $item->$field ).'" width = "30" />': '';
											if($item -> imgupload == ''){$remove = ' style="display:none;"';$upload = '';}else{$remove = '';$upload = ' style="display:none;"';}
											echo ' <a href="javascript:void(0);" class="topz-menu-upload button-secondary"'.$upload.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.esc_html__('Browse', 'topz').'</a>';
											echo ' <a href="javascript:void(0);" class="topz-menu-upload-remove"'.$remove.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.esc_html__('Remove', 'topz').'</a>';
											?>								
									<?php
										break;	
										case 'checkbox':
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s','topz'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php esc_html_e('Remove','topz'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php	echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
						?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel','topz'); ?></a>
				</div>
...
if ( !class_exists('Topz_Menu_Walker') ):

class Topz_Menu_Walker extends Walker_Nav_Menu {
...
if ( !class_exists('Topz_Mega_Menu_Walker') ):

class Topz_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if( ($depth === 0) ){
			$class_names .= '';
			$class_names .= ( $item -> mega_active == 1 ) ? ' topz-mega-menu' : ' topz-menu-custom';
			$class_names .= ( $item -> mega_full == 1 ) ? ' topz-megamenu-full' : '';
			$class_names .= ' level1';
		}		
		if( $item -> imgupload != '' ){
			$class_names .= ' topz-menu-img';
		}
		if( $item -> icon != '' ){
			$class_names .= ' topz-menu-icon';
		}
 ...
 */
function topz_nav_menu_css_class( $classes, $item ) {
	...
	return array_filter($classes, 'topz_element_empty');
}
add_filter('nav_menu_css_class', 'topz_nav_menu_css_class', 10, 2);
 ...
 * Use Topz_Menu_Walker() by default
*/
function topz_nav_menu_args($args = '') {
	$topz_nav_menu_args['container'] = false;
	$topz_theme_locates = array();
	$topz_menu = topz_options()->getCpanelValue( 'menu_location' );
	if( !is_array( $topz_menu ) ){
		$topz_theme_locates[] = $topz_menu;
	}else{
		$topz_theme_locates = $topz_menu;
	}
	if (!$args['items_wrap']) {
		$topz_nav_menu_args['items_wrap'] = '<ul class="%2$s">%3$s</ul>';
	}
	if (!$args['walker']) {
		if ( 'mega' == topz_options()->getCpanelValue( 'menu_type' ) && $topz_theme_locates[0] != NULL && in_array( $args['theme_location'], $topz_theme_locates ) ){
			$args['menu_class'] .= ' topz-mega';
			$topz_nav_menu_args['walker'] = new Topz_Mega_Menu_Walker();
			
			if( !topz_mobile_check() ) :
				wp_enqueue_script('topz_megamenu', get_template_directory_uri() . '/js/megamenu.js', array(), null, true);
			endif;
			
		} else {
			$topz_nav_menu_args['walker'] = new Topz_Menu_Walker();
		}
	}

	return array_merge($args, $topz_nav_menu_args);
}
add_filter('wp_nav_menu_args', 'topz_nav_menu_args');
